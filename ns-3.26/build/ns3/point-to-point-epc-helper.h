...
class EpcX2;
class EpcMme;
class MecServerApplication; //MiM
...
public:
  ...
  virtual Ptr<Node> GetMecNode (); //MiM
  virtual Ipv4Address GetMecAddr(); //MiM
...
private:
  ...
  /**
   * MEC server application
   */
  Ptr<MecServerApplication> m_mecServerApp;
  
  /**
   * UDP port where the GTP-U MEC socket is bound
   */
  uint16_t m_gtpMecPort;

  /**
   * TUN device for eNB - MEC tunneling 
   */
  Ptr<VirtualNetDevice> m_tunMecDevice;
