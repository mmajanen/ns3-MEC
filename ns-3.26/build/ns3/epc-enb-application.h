...
public:
  ...
  /**
   * Method to be assigned to the recv callback of the MEC socket.
   * It is called when the eNB receives a data packet from the MEC
   * that is to be forwarded to the UE.
   * \param socket  pointer to the MEC socket
   */
  void RecvFromMecSocket (Ptr<Socket> socket);
  
  /**
   * Set the MEC UDP and TCP listening sockets (MiM)
   */
  //void SetTcpMec(Ptr<Socket> socket);
  //void SetUdpMec(Ptr<Socket> socket);
  //void SetTcpSendMec(Ptr<Socket> socket);
  //void SetUdpSendMec(Ptr<Socket> socket);
  void SetMecSocket(Ptr<Socket> socket);
  void SetMecAddr(Ipv4Address addr);
  void SetMecPort(uint16_t port);
...
private:
  ...
  /**
   * Send a packet to the MEC socket (MiM)
   *
   * \param packet packet to be sent
   * \param teid the Tunnel Endpoint IDentifier
   * \return void 
   */
  void SendToMecSocket(Ptr<Packet> packet, uint32_t teid);
