...
EpcEnbApplication::RecvFromLteSocket (Ptr<Socket> socket)
{
  ...
  if (rntiIt == m_rbidTeidMap.end ())
  ...
  else
    {
      ...
      //MiM: Check if send this packet to local MEC server:
      //if destIP == MECaddr 
     
      bool useMEC = true; //whether to use MEC or not

      if(useMEC){
        Ptr<Packet> pCopy = packet->Copy ();
        Ipv4Header ipv4Header;
        pCopy->RemoveHeader (ipv4Header);
        Ipv4Address destAddr =  ipv4Header.GetDestination ();
        NS_LOG_DEBUG("IPv4 dest addr="<<destAddr);
        
        if(destAddr == m_mecAddr){
          NS_LOG_LOGIC("sending packet to MEC server application");
          NS_LOG_LOGIC("EpcEnbApplication: teid=" << teid);
          SendToMecSocket(packet, teid);
          return; //if dest != MEC --> SendToS1uSocket()
        }
      }//if(useMEC)
      
      //else, send to S1u as normally (end of MiM modifications)
      //else
...
void
EpcEnbApplication::SendToMecSocket(Ptr<Packet> packet, uint32_t teid)
{
  NS_LOG_LOGIC("SendToMecSocket(), teid=" << teid);
  GtpuHeader gtpu;
  gtpu.SetTeid (teid);
  ...
  uint32_t flags = 0;
  NS_LOG_LOGIC("m_mecAddr, m_mecPort: " << m_mecAddr << ", " << m_mecPort);
  m_mecSocket->SendTo (packet, flags, InetSocketAddress (m_mecAddr, m_mecPort));
}

void
EpcEnbApplication::SetMecPort(uint16_t port)
{
  m_mecPort = port;
}
  
void
EpcEnbApplication::SetMecAddr(Ipv4Address addr)
{
  m_mecAddr = addr;
}
  
void
EpcEnbApplication::SetMecSocket(Ptr<Socket> socket)
{
  m_mecSocket = socket;
  m_mecSocket->SetRecvCallback (MakeCallback (&EpcEnbApplication::RecvFromMecSocket, this));
}
...
void 
EpcEnbApplication::RecvFromMecSocket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);  
